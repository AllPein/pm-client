...
import { projectParticipantsSelector, tasksFilterSelector, projectTasksSelector } from '@/selectors/projectView'
...
const BoardComponent = () => {
  ...
  const mappedProjectTasks = useMemo(
    () => projectTasks.map((task) => ({
        ...task,
        key: task.id,
        asignee: participants.find((participant) => participant.id === task.asignee) ?? {}
      })),
  ...
  const board = useMemo(() => {
    return {
      columns: TaskSequence.map((taskStatus) =>
        {
          const filteredColumnCards = mappedProjectTasks.filter((task) =>
            task.status === taskStatus && (!tasksFilter.value.length || tasksFilter.value.includes(task.asignee.id))
          )
          return generateBoardColumn(TaskStatusToTitle[taskStatus], taskStatus, filteredColumnCards)
        }
      )
    }
  ...
  const handleDragCard = (card, _, destination) => {
    const newCardStatus = board.columns.find((column) => column.id === destination.toColumnId)?.status || TaskStatus.BACKLOG;
    dispatch(updateTask({
      ...card,
  ...
  return (
    <UI.Wrapper>
      <UI.BoardHeader>
        <UI.StyledInput size="large" placeholder="Поиск по доске" prefix={<SearchOutlined />} />
        <AvatarGroup
          maxCount={3}

...
const ProjectView = ({ project, userInfo }) => {
  ...
  useEffect(() => {
    dispatch(fetchUsersInProject("", project.id));
  ...
  const debouncedSetValue = useMemo(
    () =>
      debounce((value) => {
        dispatch(fetchUsersInProject(value, project.id));
  ...
  const onAssignTeamLead = useCallback(
    (participant) => {
      dispatch(
        updateParticipant({
  ...
  const getTabs = useMemo(() => {
    const tabs = [
      new Tab("participants", "Участники", () => (
        <ProjectParticipants
          userInfo={userInfo}
      ...
      new Tab("timeReport", "Работа Участников", () => (
  ...
  const onAddParticipant = useCallback(
    async (user) => {
      ...
      await dispatch(fetchUsersInProject("", project.id));
  ...
  const renderAddButton = useCallback(
    (user) => {
      ...
      return (
        <Button type="primary" onClick={() => onAddParticipant(user)}>
          Добавить
        </Button>
      );
    },
